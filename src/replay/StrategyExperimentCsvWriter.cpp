...
bool StrategyExperimentCsvWriter::Write(const std::string &path,
    ...
    for (const auto &result : results) {
        out << EscapeCsv(StrategyModeToString(result.mode)) << "," << result.entry_offset << ","
            << (result.is_buy ? "true" : "false") << "," << result.limit_price << ","
            << result.requested_qty << "," << result.filled_qty << "," << result.remaining_qty
            << "," << result.fill_rate << "," << result.avg_execution_price << ","
            << result.decision_mid_price << "," << result.decision_spread << ","
            << result.implementation_shortfall_bps << "," << result.time_to_first_fill_us << ","
